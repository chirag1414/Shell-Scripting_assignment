...
echo  "Please enter your components ? INGESTOR/JOINER/WRANGLER/VALIDATOR?"
read input1
if [[ $input1 == "INGESTOR" || $input1 == "JOINER" || $input1 == "WRANGLER" || $input1 == "VALIDATOR" ]]; then
        echo "Component you entered is:" $input1
else
...
echo "Please enter your scale ? MID/HIGH/LOW?"
read input2
if  [[ $input2 == "MID" || $input2 == "HIGH" || $input2 == "LOW"  ]]; then
        echo "Scale you have entered is:" $input2
else
...
echo "Please enter the count ? 1-9?"
read input4
if  [[ "$input4" =~ ^[1-9]$ ]]; then
	echo "$current_time Error: Range <1-9>" >> errors.log
       	echo "Count number is:" $input4
...
if [[ $err == 0 ]]; then
	echo "$current_time Invalid Input" >> errors.log
	echo "Error: The inputs you have entered does not match the given file."
	exit
fi
